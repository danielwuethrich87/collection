...
blastn -db "$working_dir"/../../db/SILVA_128_SSUParc_tax_silva_trunc_filtered.fasta -num_threads "$cores" -max_target_seqs 250 -query "$working_dir"/results/reads/"$i"_16s_reads.fasta -out "$working_dir"/results/blastn/"$i".results.csv -outfmt "6 qseqid sseqid stitle qlen slen length pident nident mismatch gaps evalue bitscore"
